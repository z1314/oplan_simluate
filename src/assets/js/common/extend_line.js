...
export default class ExtendLine extends SuperChart {
  ...
  line_biax(chartData) {
    ...
    if (chartData.yAxis) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (chartData.forecastYA) {
          this.seriesForecast = this.seriesForecast.concat(
            chartData.forecastYA[i]
          );
        }
      }
    } else {
    ...
    this.option.series.push(
      ...
      },
      {
        ...
        data: this.seriesData.concat(
          this.seriesForecast.slice(
            this.seriesData.length,
            this.seriesForecast.length
          )
        )
      }
    );
  ...
  showChart(config) {
    ...
    if (config.axis == 1) {
      ...
      //进入双轴图表
      if (config.showType == "continuous") {
        //多日连续
        this.line_biax(config.data);
      } else if (config.showType == "overlay") {
        //多日叠加
